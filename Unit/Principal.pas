...
begin
  ...
  vAux := Trim(vAux.Replace('GROUP BY', sLineBreak + ' GROUP BY ')); // Faz quebra de linha no LEFT JOIN
  ...
  begin
    vTextoLinha := vTextoLinha + '{' + FormatFloat('000', x) + '}  ''' + Trim(memo2.Lines.Strings[i]) + ' '' + ' + sLineBreak;
    x := x + 1;
  end;
